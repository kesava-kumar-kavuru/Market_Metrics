          ...
          <>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <div className={`flex items-center space-x-1 ${
                        companyData.priceChange >= 0 ? 'text-emerald-400' : 'text-red-400'
                      }`}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <CardHeader className="pb-2">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex items-center gap-3 text-xs">
                        ...
                        <div className="flex items-center gap-1.5">
                          <div className="w-3 h-0.5 bg-purple-400 rounded-full" style={{ borderStyle: 'dashed' }} />
                  ...
                  <div className="flex items-center gap-6 mt-3 pt-3 border-t border-border/30">
                    <div>
                      ...
                      <p className="text-lg font-bold text-foreground">${companyData.currentPrice?.toFixed(2)}</p>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">Predicted</span>
                      <p className="text-lg font-bold text-purple-400">
                        ${chartData.length > 0 && chartData[chartData.length - 1]?.predicted 
                          ? chartData[chartData.length - 1].predicted.toFixed(2) 
                          : '--'}
                      </p>
                    </div>
                    <div>
                      <span className="text-xs text-muted-foreground">Change</span>
                      <p className={`text-lg font-bold ${companyData.priceChange >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                        {companyData.priceChange >= 0 ? '+' : ''}{companyData.changePercent?.toFixed(2)}%
                      </p>
                    </div>
                    <div className="ml-auto">
                      <span className="text-xs text-muted-foreground">Confidence</span>
                      <p className="text-lg font-bold text-amber-400">{companyData.insights?.confidence || '--'}%</p>
                    </div>
                  </div>
                </CardHeader>
              ...
              <Card className="bg-gradient-to-br from-card to-card/80 border-border/50">
                <CardHeader>
                  <CardTitle className="text-lg text-foreground">Trading Insights</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="p-4 rounded-lg bg-primary/5 border border-primary/20">
                    ...
                    <div className={`text-2xl font-bold ${
                        companyData.insights.signal === 'BUY' ? 'text-emerald-400' : 'text-red-400'
                    }`}>
                  ...
                  <div className="p-4 rounded-lg bg-warning/5 border border-warning/20">
                    ...
                    <div className={`text-2xl font-bold ${
                        companyData.insights.risk === 'High' ? 'text-red-400' : 
                        companyData.insights.risk === 'Medium' ? 'text-amber-400' : 'text-emerald-400'
                    }`}>
